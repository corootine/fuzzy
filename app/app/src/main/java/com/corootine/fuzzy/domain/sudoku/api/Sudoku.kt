 ...
 * * No changes are allowed to fields which are already given (clues). See [Cell.clue].
 *
...
interface Sudoku : Iterable<Sudoku.Cell> {
     ...
     * @throws IndexOutOfBoundsException if row or column are out of range.
     * ```
     ...
     * @throws IndexOutOfBoundsException if value, row or column are out of range.
     * ```
     * row >= 0 && row < PuzzleGenerator.Metadata.rowsInGrid
     * column >= 0 && column < PuzzleGenerator.Metadata.columnsInGrid
    ...
    data class Cell(val row: Int, val column: Int, val value: Int, var clue: Boolean) {
        ...
        /**
         * Checks if the cell contains a value.
         *
